...
function backendOrigin() { return (process.env.NEXT_PUBLIC_API_ORIGIN || process.env.VITE_API_ORIGIN || "").replace(/\/$/, ""); }
async function forward(request: NextRequest, { params }: { params: { path: string[] } }) {
  const origin = backendOrigin();
  if (!origin) return NextResponse.json({ message: "Backend API is not configured." }, { status: 503 });
  const target = `${origin}/api/${params.path.map((part) => encodeURIComponent(part)).join("/")}${request.nextUrl.search}`;
